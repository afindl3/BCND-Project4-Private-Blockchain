...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const lastBlock = await self.getBlockByHeight(height);
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if ((currentTime - messageTime) < (10 * 60)) { // CHANG BACK TO 5
        const isValid = bitcoinMessage.verify(message, address, signature);
        if (isValid) {
          const newBlock = new BlockClass.Block({ star });
          newBlock.owner = address;
          const addedBlock = await self._addBlock(newBlock);
          resolve(addedBlock);
        } else {
          reject("Message not valid");
        }
      } else {
        reject("More than 5 minutes has elapsed since requesting message");
      }
    });
  }
  ...
  validateChain() {
    ...
    let errorLog = [];
    return new Promise(async (resolve, reject) => {
      const height = await self.getChainHeight(); // Could also use chain.length
      let previousHash = '';
      ...
      for (let i = 0; i < height; i++ ) {
        ...
        if (!isValid) {
          ...
          const block = JSON.parse(self.chain[i]);
          if (i === 0) {
            previousHash = block.hash;
          } else if (block.previousBlockHash !== previousHash) {
            errorLog.push(`Invalid block link (previousBlockHash does not match the previous blocks hash), block: ${i}`);
            previousHash = block.hash;
          }
        }
      }
